...
def build_chat_messages(config, example):
    """Build a system/user chat message list for split (chat) mode.

    system_prompt (when set) becomes a system-role turn and user_prompt a
    user-role turn, each rendered per row via format_template_prompt (same
    renderer as flat mode). The assistant turn is generated by the model.
    """
    messages = []
    if config.system_prompt:
        messages.append({
            "role": "system",
            "content": format_template_prompt(config.system_prompt, example, config),
        })
    messages.append({
        "role": "user",
        "content": format_template_prompt(config.user_prompt, example, config),
    })
    return messages
    ...
    for checkpoint_idx, checkpoint_path in enumerate(checkpoint_paths):
        ...
        for i, example in enumerate(data_split):
            ...
            if config.split_mode:
                messages = build_chat_messages(config, example)
                system_text = messages[0]["content"] if messages[0]["role"] == "system" else None
            ...
            try:
                ...
                if use_api:
                    response = generate_response_by_api(
                        prompt=user_text if messages is not None else full_prompt,
                        config=config,
                        system_prompt=system_text if messages is not None else None,
                    )
                elif is_vlm:
